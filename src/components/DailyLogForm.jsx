...
export default function DailyLogForm({ initialValue, onSubmit, submitting }) {
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      <label className="flex flex-col gap-1">
        <span className="text-sm font-medium text-slate-700">Project(s) worked on (comma-separated) *</span>
        <input
          type="text"
          value={form.projects}
          required
      ...
      <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-100 p-3">
        <h2 className="text-sm font-semibold text-slate-800">Daily self-evaluation (1-5)</h2>
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full rounded-lg bg-blue-700 px-4 py-3 text-sm font-semibold text-white disabled:bg-blue-400"
      >
